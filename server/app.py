...
def insert_default_users():
    if User.query.count() == 0:
        ...
        mariorossi = User(email="mariorossi@progettobul.com", name="Mario Rossi", password=hashed_password) 
        darioverdi = User(email="darioverdi@progettobul.com", name="Dario Verdi", password=hashed_password, roleid=1) 
        db.session.add(mariorossi)
        db.session.add(darioverdi)
        db.session.commit()
